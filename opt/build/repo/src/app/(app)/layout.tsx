...
export default function AppLayout({
  ...
  if (isLoading || (!session && pathname !== '/login' && pathname !== '/signup')) {
    if (isLoading) {
        return (
            // Aplicando overflow-hidden aqui também para o esqueleto
  ...
  return (
    <SidebarProvider defaultOpen>
      <div className="flex min-h-screen flex-col bg-background overflow-hidden"> {/* Adicionado overflow-hidden */}
        <AppHeader />
        <div className="flex flex-1 pt-16"> 
          <AppSidebar />
          <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8">
