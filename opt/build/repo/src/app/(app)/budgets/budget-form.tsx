...
export function BudgetForm({ onFormSuccess, initialData, isModal = true }: BudgetFormProps) {
  ...
  const onBudgetSubmit: SubmitHandler<BudgetFormData> = async (data) => {
    ...
    toast({
      title: isEditing ? "Orçamento Atualizado! (Simulação)" : "Orçamento Criado! (Simulação)",
  ...
  const onCategorySubmit: SubmitHandler<NewCategoryFormData> = async (data) => {
    ...
    if (error) {
      ...
      toast({
        ...
        description: `Categoria "${data.name}" criada.`
      });
    ...
    <form onSubmit={handleSubmit(onBudgetSubmit)} className="space-y-6">
                ...
                <SelectContent>
                  ...
                  {categories.length === 0 && !isLoadingCategories && (
                    <div className="p-4 text-sm text-center text-muted-foreground">
                      Nenhuma categoria de despesa encontrada. Crie uma!
                    </div>
                  )}
                </SelectContent>
              </Select>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <Input id="limit_amount" type="number" step="0.01" {...register("limit_amount")} className="pl-10" />
        </div>
        {errors.limit_amount && (
          <p className="text-sm text-destructive mt-1">{errors.limit_amount.message}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            render={({ field }) => (
              <Popover>
                ...
                <PopoverContent className="w-auto p-0">
                  <Calendar mode="single" selected={field.value as Date} onSelect={field.onChange} initialFocus locale={ptBR} />
                </PopoverContent>
              </Popover>
            )}
          />
          {errors.period_start_date && (
            <p className="text-sm text-destructive mt-1">{errors.period_start_date.message}</p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          {errors.period_end_date && (
            <p className="text-sm text-destructive mt-1">{errors.period_end_date.message}</p>
          )}
        </div>
      </div>
