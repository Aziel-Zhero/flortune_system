...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    const simulatedFetchedEvents = assignEventsToCurrentWeek(
      sampleBaseEvents,
      weekDates
    );
  ...
  const calculateEventStyle = (
    ...
    const topPosition =
      (startH - timelineStartHour + startM / 60) * slotHeight;
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button
              size="sm"
              onClick={() => alert("Funcionalidade Adicionar Evento (placeholder)")}
            >
      ...
      <div className="flex-1 grid grid-cols-[60px_repeat(7,1fr)] border bg-card rounded-lg shadow-sm overflow-hidden">
        ...
        <div className="col-span-1 row-span-1 border-r border-b p-2 text-xs text-muted-foreground sticky top-0 bg-card z-10"></div>{" "}
        {/* Canto superior esquerdo */}
        {weekDates.map((date, i) => (
          <div
            key={`header-${i}`}
            className={cn(
              "row-span-1 p-2 text-center border-b sticky top-0 bg-card z-10",
              i < 6 && "border-r"
            )}
          >
            <div className="text-xs text-muted-foreground font-medium">
              {weekDayLabels[getDay(date)]}
            </div>
            <div
              className={cn(
                "text-xl font-semibold mt-1",
                fnsIsToday(date) ? "text-primary" : "text-foreground"
              )}
            >
              {format(date, "d")}
            </div>
        ...
        {timeSlots.map((hour, hourIndex) => (
          <React.Fragment key={`timeslot-row-${hour}`}>
            ...
            <div
              className={cn(
                "col-start-1 row-start-${hourIndex + 2} h-[60px] border-r text-right pr-2 text-xs pt-1 text-muted-foreground sticky left-0 bg-card z-10",
            ...
            {weekDates.map((date, dayIndex) => (
              <div
                ...
                className={cn(
                  `col-start-${dayIndex + 2} row-start-${
                    hourIndex + 2
                  } relative h-[60px]`,
                  dayIndex < 6 && "border-r",
                  hourIndex < timeSlots.length - 1 && "border-b"
              ...
              >
                ...
                {events
                  .filter(
                    (event) =>
                      isSameDay(parseISO(event.date), date) && !event.isAllDay
                  )
                  .map((event) => {
                    ...
                    if (
                      startH < timeSlots[0] ||
                      endH > timeSlots[timeSlots.length - 1] + 1
                    )
                      return null; // Fora do range visível
        ...
        <div className="col-start-2 col-span-7 row-start-2 grid grid-cols-7 relative h-[30px] pointer-events-none">
          {weekDates.map((date, dayIndex) => (
            <div
              ...
              className={cn("relative", dayIndex < 6 && "border-r")}
            >
              <div className="absolute top-0 left-0 right-0 p-0.5 space-y-0.5 pointer-events-auto">
                {events
                  ...
                  .map((event) => {
                    const { height, zIndex } = calculateEventStyle(
                      event.startTime,
                      event.endTime,
                      event.isAllDay
                    );
      ...
      {selectedEvent && (
        ...
        >
          <DialogContent
            className={cn(
              "sm:max-w-md",
              selectedEvent.color?.replace("bg-", "border-t-4 border-") ||
          ...
          >
            ...
            <div className="space-y-3 py-4 text-sm">
              ...
              {selectedEvent.organizer && (
                <p>
                  <strong className="text-foreground/80">Organizador:</strong>{" "}
                  {selectedEvent.organizer}
                </p>
              )}
            </div>
            <DialogFooter>
              ...
              <Button
                type="button"
                variant="default"
                onClick={() => alert("Funcionalidade Editar Evento (placeholder)")}
              >
                Editar
              </Button>
            </DialogFooter>
